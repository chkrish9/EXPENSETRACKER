...
function Expense() {
  ...
  const onSubmit = (expenseDetails) => {
    expenseDetails = {
      ...expenseDetails,
      category:
        typeof expenseDetails.category === "string"
          ? getCode(expenseDetails.category)
          : expenseDetails.category.code,
      date: new Date(expenseDetails.date).toLocaleDateString(),
      subCategory:
        typeof expenseDetails.subCategory === "string"
          ? getCode(expenseDetails.subCategory)
          : expenseDetails.subCategory.code,
      account:
        typeof expenseDetails.account === "string"
          ? getCode(expenseDetails.account)
          : expenseDetails.account.code,
    };
  ...
  return (
    <>
      <h1>Total Expense:{numberFormat(totalExpenses())}</h1>
      <div className="card">
      ...
      <TransactionForm
        ...
        accountOptions={getTransactionsCategories("expenses", "account")}
        type={"expenses"}
        transaction={expense}
      />
      <Button
